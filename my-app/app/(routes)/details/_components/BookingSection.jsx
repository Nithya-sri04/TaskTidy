...
function BookingSection({ children, business }) {
    ...
    const BusinessBookedSlot = () => {
        GlobalApi.BusinessBookedSlot(business.id, moment(date).format('DD-MMM-yyyy'))
            .then(resp => {
                console.log('BusinessBookedSlot Response:', resp);
                setBookedSlot(resp.bookings || []);
            })
            .catch((error) => {
                console.error('Error in BusinessBookedSlot:', error);
            });
    };
    ...
    const saveBooking = () => {
        ...
        GlobalApi.createNewBooking(business.id,
            ...
            .then(resp => {
                console.log('API Response:', resp); // Log the entire API response
                
                // Check if response and createBooking are defined
                if (resp && resp.createBooking) {
                    setDate(new Date());
                    setSelectedTime('');
                    toast('Service Booked successfully!');
                    console.log('Booking ID:', resp.createBooking.id); // Log the booking ID
                } else {
                    toast('Service booked, but no booking ID returned.');
                }
            })
            .catch((error) => {
